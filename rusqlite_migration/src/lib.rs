...
impl<'m> Migrations<'m> {
    ...
    /// Returns the number of migrations that would be applied by [`Migrations::to_latest`].
    /// The number returned may be negative, if more migrations were applied than the current
    /// version of the program knows about.
    ///
    ...
    /// ]);
    ///
    /// if migrations.migrations_to_apply(&conn).unwrap() != 0 {
    ///     // Backup the database
    ///
    ///     migrations.to_latest(&mut conn).unwrap()
    /// }
    ...
    /// assert_eq!(migrations_v1.migrations_to_apply(&conn), Ok(0));
    ///
    ...
    /// // reversed.
    /// assert_eq!(migrations_v1.migrations_to_apply(&conn), Ok(-1));
    ...
    /// )));
    /// ```
    pub fn migrations_to_apply(&self, conn: &Connection) -> Result<i32> {
